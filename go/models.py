...
from go.utils import get_deep_hash, deep_equals, transform_indexes, get_position2index
...
stones_plot = np.full((20, 20), None)
...
class Board:
    ...
    def play_in_board(self, point, player, handle_ko):
        ...
        if point.group is not None:
            return False
        self.captured_stones, captured_groups = None, None
        if handle_ko is True:
            self.captured_stones, captured_groups = set(), set()
        ...
        for group in adj_groups:
            if id(group.owner) == id(player):
                ...
                if len(group.liberties) == 0:
                    if handle_ko:
                        self.captured_stones.update(group.stones)
